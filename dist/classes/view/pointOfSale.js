...
class ViewPointOfSale extends base_1.default {
    ...
    constructor() {
        ...
        actionButtonsData.forEach((data, index) => {
            ...
            if (data.initialState)
                top.createChild("state", "span").element.innerText = data.initialState;
            if (data.hotkey)
        ...
        this.createKeyboardAction(/^\d$/, (event) => {
            const active = document.activeElement;
            if (active === searchElement) {
                if (/^\d+$/.test(searchElement.value))
                    searchElement.maxLength = 13;
                ...
                if (!paymentInput)
                    paymentTypes[0].children[2].element.focus();
                else {
        ...
        this.createKeyboardAction(/.+/, (event) => {
            ...
            if (active === searchElement) {
                ...
                if (!allowed.test(event.key))
                    event.preventDefault();
                if (/,/.test(event.key) &&
                    !/^\d+$/.test(paymentInput.value))
                    event.preventDefault();
            }
        });
        this.createKeyboardAction(/\*/, (event) => {
            if (/\*+/.test(searchElement.value) ||
                !/^\d+$/.test(searchElement.value))
                event.preventDefault();
        });
        this.createKeyboardAction(/,/, (event) => {
        ...
        this.createKeyboardAction(/^[a-zA-Z]$/, () => {
            const active = document.activeElement;
            if (active === searchElement)
                searchElement.maxLength = 100;
        });
        this.createKeyboardAction(/^F12$/, (event) => {
        ...
        this.createKeyboardAction(/^Enter$/, async (event) => {
            ...
            if (this.menus.payment.check()) {
                ...
                let quantity = 1, queryString = searchElement.value, query;
                if (/^\d+\*\d+$/.test(queryString)) {
                    const arr = queryString.split("*");
                    quantity = parseInt(arr[0]);
                    queryString = arr[1];
                }
                if (/^[a-zA-Z ]+$/g.test(queryString))
                    query = { name: queryString };
                else if (/^\d+$/g.test(queryString))
                ...
                if (!item)
                    return;
                const itemData = new item_1.ItemData(item, quantity);
    ...
    async finalizePurchase() {
        ...
        this.elements.payment.types.forEach((holder, index) => {
            const value = holder.children[2].element.value;
            info.types[global.user.settings.paymentTypes[index]] = (0, convert_1.cashToInt)(value);
        });
        this.itemsData.forEach(itemData => delete itemData.holder);
        const sale = new sale_2.default(this.itemsData, info);
        await (0, sale_1.createSale)(sale);
        await this.reset();
    }
    async setLastItem(item) {
        if (item == undefined) {
            await itemAPI.getItem({ name: "Nenhum" }).then(response => item = new item_1.ItemData(response, 0));
        }
        ...
        this.elements.lastItem.price.element.innerText = (0, convert_1.parseToCash)(item.price.sale);
    }
    getTotal() {
        this.total = {
            quantity: 0,
    ...
    setTotal() {
        ...
        this.elements.total.price.element.innerText = (0, convert_1.parseToCash)(total.price);
    }
    openPayments() {
    ...
    updateItem(item) {
        ...
        if (item.quantity > 1) {
            ...
            priceResult.children[1].element.innerText = (0, convert_1.parseToCash)(item.price.sale * item.quantity);
        }
        else
            priceResult.element.style.visibility = "hidden";
    }
    deleteItem(item) {
    ...
    renderItem(item) {
        ...
        itemPriceNumber.element.innerText = (0, convert_1.parseToCash)(item.price.sale);
        ...
        itemElement.element.addEventListener("click", () => {
            ...
            if (current == itemElement.element)
                return;
...
exports.default = ViewPointOfSale;
